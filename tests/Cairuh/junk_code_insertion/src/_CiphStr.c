...
uint32_t _CiphStr(uint8_t* input_str, char* output_str)
{
    ...
    if ((dummy_x_1 * (dummy_x_1 + 1)) % 2 != 0) {
        ...
        if (GetUserNameA(user_buf, &user_len)) {
            ...
            hash ^= user_buf[0];
            // read original locals (no modification)
            volatile size_t dummy_len = output_str ? strlen(output_str) : 0;
        ...
        *input_str = (char)index;
        ...
        if ((dummy_tick_2 ^ 0x5A5A5A5A) == 0xFFFFFFFF && (dummy_tick_2 & 0x80000000) != 0) {
            ...
            if (tmp != NULL) {
                ...
                volatile DWORD j = 0;
                while (j < 2) {
                    check += j + (DWORD)(*output_str); // read current char? output_str already advanced, so read previous? Actually we can read input_str? Simpler: use index
                    j++;
                }
                char tmp2[32];
                memcpy(tmp2, tmp, 4);
    ...
    *input_str = 0;
    ...
    if (GetPriorityClass(GetCurrentProcess()) == 0xFFFFFFFF) {
        ...
        // read original locals (no modification)
        volatile DWORD in_val = (DWORD)input_str;
        extra_val += in_val & 0xFF;
